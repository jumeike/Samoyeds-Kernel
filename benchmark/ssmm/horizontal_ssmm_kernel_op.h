...
__global__ void _HorizontalSsmmKernel(const int m, const int n, const int k,
    ...
    kernel.mainLoop(m, n, k, logical_N, logical_M, // Pass logical N, M to kernel
                    0, 0, A_values, A_metadata, A_indices, B, B_indices, B_indices_len,
...
__global__ void _HorizontalSsmmTransKernel(const int m, const int n, const int k,
    ...
    kernel.mainLoopTrans(m, n, k, logical_N, logical_M, // Pass logical N, M to kernel
                        0, 0, A_values, A_metadata, A_indices, B, B_indices, B_indices_len,
...
//             m, n, k, A_values, A_metadata, A_indices, B, B_indices, B_indices_len, C, D);
    ...
    if (N == 1 && M == 4) {
        ...
        size_t shared_mem_size = max(KernelType::input_buffer_size, KernelType::output_buffer_size);
        int dim_x = m / KernelType::Block_M / M * N;
        // int dim_x = (m * N) / (KernelType::Block_M * M);
        int dim_y = (B_indices_len + KernelType::Block_N - 1) / KernelType::Block_N;
        // printf("[_HorizontalSsmmKernel LAUNCH] dim_x=%d, dim_y=%d, m=%d, N=%d, M=%d\n", dim_x, dim_y, m, N, M);
        _HorizontalSsmmKernel<KernelType><<<dim3(dim_x, dim_y, 1), dim3(128, 1, 1), shared_mem_size, stream>>>(
                m, n, k, N, M, A_values, A_metadata, A_indices, B, B_indices, B_indices_len, C, D);
    } else if (N == 2 && M == 4) {
        using KernelType = HorizontalSsmmKernel<SparseRatioBase<2, 4>, 128, BlockShape, WarpShape, MmaShape, NStage,
                AccumulatorType, ASwizzle, BSwizzle, CSwizzle>;
        size_t shared_mem_size = max(KernelType::input_buffer_size, KernelType::output_buffer_size);
        int dim_x = m / KernelType::Block_M / M * N;
        // int dim_x = (m * N) / (KernelType::Block_M * M);
...
//             m, n, k, A_values, A_metadata, A_indices, B, B_indices, B_indices_len, C, D);
    ...
    if (N == 1 && M == 4) {
        ...
        // int dim_x = (m * N) / (KernelType::Block_M * M);
        int dim_y = (B_indices_len + KernelType::Block_N - 1) / KernelType::Block_N;
        // printf("[_HorizontalSsmmTransKernel LAUNCH] dim_x=%d, dim_y=%d, m=%d, N=%d, M=%d\n", dim_x, dim_y, m, N, M);
        _HorizontalSsmmTransKernel<KernelType><<<dim3(dim_x, dim_y, 1), dim3(128, 1, 1), shared_mem_size, stream>>>(
                m, n, k, N, M, A_values, A_metadata, A_indices, B, B_indices, B_indices_len, C, D);
    } else if (N == 2 && M == 4) {
        using KernelType = HorizontalSsmmKernel<SparseRatioBase<2, 4>, 128, BlockShape, WarpShape, MmaShape, NStage,
...
//             m, n, k, A_values, A_metadata, A_indices, B, B_indices, B_indices_len, C, D);
    ...
    if (N == 1 && M == 4) {
        ...
        // int dim_x = (m * N) / (KernelType::Block_M * M);
        int dim_y = (B_indices_len + KernelType::Block_N - 1) / KernelType::Block_N;
        // printf("[_HorizontalSsmmFusedActTransKernel LAUNCH] dim_x=%d, dim_y=%d, m=%d, N=%d, M=%d\n", dim_x, dim_y, m, N, M);
        _HorizontalSsmmFusedActTransKernel<KernelType><<<dim3(dim_x, dim_y, 1), dim3(128, 1, 1), shared_mem_size, stream>>>(
                m, n, k, N, M, A_values, A_metadata, A_indices, B, B_indices, B_indices_len, C, D);
    } else if (N == 2 && M == 4) {
        using KernelType = HorizontalSsmmKernel<SparseRatioBase<2, 4>, 128, BlockShape, WarpShape, MmaShape, NStage,
